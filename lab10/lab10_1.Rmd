...
homerange %>% 
  ...
  labs(title = "Thermoregulation Type", x= "Type of Thermoregulation", y="Amount")+ theme(plot.title = element_text(size = rel(1.5), hjust = 0.5))
...
```{r}
homerange %>% 
  ggplot(aes(x=log10.hra, y=log10.mass, size=log10.mass))+geom_point(na.rm=T) + labs(title = "Homerange Area versus Mass", x="Homerange Area (log10)", y="Mass (log10)") + theme(plot.title = element_text(size = rel(1.5), hjust = 0.5))
```
