...
homerange %>% 
  ...
  geom_histogram(alpha = 0.4, color = "turquoise", fill = "springgreen")+ #can also add bin="_" to adjust how it looks
...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_density(fill="salmon3", alpha  =0.4, color = "palevioletred1")+
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ...
  geom_histogram(aes(y= ..density..),fill="wheat", alpha= 0.4, color="saddlebrown")+geom_density(color= "palevioletred")+
...
```{r}
homerange<- homerange %>% 
  mutate(range_category = case_when(log10.hra <= 1.0 ~"very small",
                                    log10.hra > 1.0 & log10.hra <= 3.6 ~"small",
                                    log10.hra > 3.6 & log10.hra <= 6.0 ~"medium",
                                    log10.hra > 6.0 ~"large"))
```
```{r}
...
homerange %>%
  ...
  labs(title= "Mass Range of Small Animal Classes")
```
