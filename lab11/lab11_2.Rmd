...
`facet_wrap()` makes a ribbon of panels, but you can control how you want them arranged.  #used for 1 variables
```{r}
ecosphere %>% 
  ggplot(aes(x=diet, y=log10_mass, fill=migratory_strategy))+ 
  geom_boxplot(alpha=0.4) + 
...
`facet_grid()` allows control over the faceted variable; it can be arranged in rows or columns. # used for 2+ more variables
```{r}
ecosphere %>% 
  ggplot(aes(x=diet, y=log10_mass, fill=migratory_strategy))+ 
  geom_boxplot(alpha=0.4)+ 
...
```{r}
ecosphere %>% 
  ggplot(aes(x=mean_eggs_per_clutch))+geom_density()+facet_grid(.~habitat)
```
