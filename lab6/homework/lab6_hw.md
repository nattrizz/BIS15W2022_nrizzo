...
```r
compare_df_cols(fisheries)
```

```
##                column_name fisheries
## 1                  country character
## 2              common_name character
## 3     isscaap_group_number   numeric
## 4  isscaap_taxonomic_group character
## 5     asfis_species_number character
## 6       asfis_species_name character
## 7   fao_major_fishing_area   numeric
## 8                  measure character
## 9                    x1950 character
## 10                   x1951 character
## 11                   x1952 character
...
```r
fisheries_tidy %>%
  select(common_name, country, catch) %>% 
  filter(country=="Germany") %>%
  summarise(ger_catch=sum(catch, na.rm = T))
```
